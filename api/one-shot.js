// api/one-shot.js — pick random sprite/background from Supabase, save in-memory game, redirect to /play.html?slug=...
...
module.exports = async (req, res) => {
  try {
    ...
    let bgs = [];
    for (const cand of BG_CANDIDATES) { const rows = await supaList(cand); if (rows.length) { bgs = rows; break; } }
    ...
    const s = sprites[Math.floor(Math.random()*sprite*]()
