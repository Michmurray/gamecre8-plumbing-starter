  ...
  try {
    ...
    if (useSupa) {
      ...
      if (!sprites.length || !bgs.length) {
        return res.status(200).json({ ok: false, error: 'Assets missing', counts: { sprites: sprites.length, backgrounds: bgs.length } });
      }
      ...
      if (!s || !b) return res.status(200).json({ ok: false, error: 'Assets missing', counts: { sprites: s ? 1 : 0, backgrounds: b ? 1 : 0 } });
      art = { sprite: 'sprite/' + s, background: 'backgrounds/' + b };
    }

    const slug = slugify(prompt);
    const s = store();
    s.games[slug] = { slug, prompt, art, created_at: new Date().toISOString() };
