// api/self-test.js — E2E diag: env → Supabase list → counts → one-shot → get-game → queue.
...
function postJSON(urlStr, headers, body) {
  return new Promise((resolve, reject) => {
    try {
      ...
      const req = https.request(opts, (res) => { let buf=''; res.on('data',d=>buf+=d); res.on('end',()=>{ let j=null; try{ j=JSON.parse(buf||'null'); }catch{} resolve({status:res.statusCode,json:j,text:buf,headers:res.headers}); });});
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    set('env', envOk, { ASSETS_SOURCE, SUPABASE_URL_HOST: SUPABASE_URL.replace('https://',''), SUPABASE_ANON_KEY_MASKED: mask(SUPABASE_ANON_KEY), SUPABASE_BUCKET, SPRITES_PREFIX, BG_CANDIDATES });
    ...
    async function supaList(prefix){
      const { json } = await postJSON(listEndpoint, { apikey:SUPABASE_ANON_KEY, authorization:`Bearer ${SUPABASE_ANON_KEY}` },
    ...
    for (const cand of BG_CANDIDATES) { const r = await supaList(cand); if (r.length) { bgRows=r; usedBg=cand; break; } }
    ...
    let oneSlug=null; try { if (loc) oneSlug = new URL(loc, base).searchParams.get('slug'); } catch {}
    ...
    const qe = await postJSON(`${base}/api/queue-prompt`, { 'content-type':'application/json' }, { prompt:`self-test-queue-${now}`, redirect:0 });
    ...
    try { if (qLoc) queueSlug = new URL(qLoc, base).searchParams.get('slug'); } catch {}
    if (!queueSlug && qr.json && qr.json.slug) queueSlug = qr.json.slug;
    set('queue_run', !!queueSlug, { status:qr.status, location:qLoc||'', body:qr.json || qr.text, slug:queueSlug });

    const allGood = out.summary.env && out.summary.asset_counts && out.summary.one_shot && out.summary.get_game && out.summary.queue_enqueue && out.summary.queue_run;
    out.ok = !!allGood; out.summary.ready = allGood;
